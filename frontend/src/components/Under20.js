...
const Under20 = () => {
    ...
    const filterShowHandler = () => SetFilter(!filter);
    const toggleSortHandler = () => SetSort(!sort);
    ...
    const bodyLotionProducts = store.filter(product => product.type.includes('bodyLotion'));
    const bodyScrubProducts = store.filter(product => product.type.includes("bodyScrub"));
    const bodyWashProducts = store.filter(product => product.type.includes("bodywash"));

    const bgAddHandler = (e) => e.target.classList.add("whi");
    const bgRemoveHandler = (e) => e.target.classList.remove("whi");
            ...
            <div className='filterOptionsHold relative'>
                {filter && (
                    <div className='flex rounded-xl gap-8 flex-col boxSh fof absolute'>
                        <p className='ml-12 scale cursor-pointer' onClick={bodyLotionHandler}>Body Lotion</p>
