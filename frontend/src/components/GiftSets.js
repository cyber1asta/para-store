    ...
    return (
        <div className='giftSetMainParent'>
            ...
            <div className='flex flex-col gap-8 bestSellerPriceHolder'>
                <Link to={"/under20"} >
                    <p className="hover:underline"> Under DH 210 </p>
                </Link>

                <Link to={"/under40"} >
                    <p className="hover:underline"> Under DH 420 </p>
                </Link>
            ...
            <div className='flex flex-row gap-20 giftHold'>
                {store.map((item) => {
                    if (item.type === "gift")
                        return (
                            <Link to={`/${item.id}`} key={item.id}>
                                <div className="giftIndivitual">
                                    ...
                                    <p className='giftPrice font-normal text-center'>
                                        {`DH ${new Intl.NumberFormat('ar-MA', { minimumFractionDigits: 2 }).format(item.price * 10.5)}`}
                                    </p>
                                </div>
                            </Link>
                        );
