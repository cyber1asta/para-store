...
const SinglePage = () => {  
  ...
  // تحويل السعر إلى درهم مغربي  
  const conversionRate = 10.40; // سعر الصرف  
  const priceInMAD = (price * conversionRate).toFixed(2); // تحويل السعر  
  ...
  return (  
    <div className='singlePageMainParent relative top-36'>  
      ...
      <figure className='singlePageMainPicHold relative'>  
        {imgChange ? (  
          <img src={activeImg} className=" w-48 cursor-pointer rounded-2xl object-cover singlePageMainPic" />  
        ) : (  
          <img src={image} className=" w-48 cursor-pointer rounded-2xl object-cover singlePageMainPic" />  
        )}  
      </figure>  
      ...
      <div className='namePriceSP relative fof flex flex-col gap-6'>  
        ...
        <p className='text-xl sp relative'> MAD {priceInMAD} </p> {/* عرض السعر بالدرهم المغربي */}  
      </div>  
      ...
      <div className='relative gap-2 flex starHold'>  
        {Array.from({ length: numOfRev }).map((_, i) => (  
          <div className='flex' key={i}> <AiFillStar /> </div>  
        ))}  
        <p className='font-semibold l1rem'>{reviews} reviews </p>  
      </div>  

      <p className='font-semibold text-md relative loyalty'> YOU CAN EARN 50 LOYALTY POINTS </p>  
